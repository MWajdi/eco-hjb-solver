...
class PIATrainer:
    ...
    def get_V_and_alpha(self, t, P, Y):
        ...
        alpha_tilde = self.g_phi(t_norm, tf.concat([P_norm, Y_norm], axis=1))
    ...
    def compute_value_loss(self, t, P, Y, t_term, P_term, Y_term):
        ...
        drift_coeff = self.alpha_scale * mu
        diffusion_coeff = (self.alpha_scale**2 * sigma**2) / tf.constant(2.0, dtype=self.dtype)
        ...
        residual = self.V_scale * (V_tilde_t + drift + diffusion + advec_Y + source_term - r * V_tilde)
        pde_loss = tf.reduce_mean(tf.square(residual))

        t_term_norm, P_term_norm, Y_term_norm = self.normalize_inputs(t_term, P_term, Y_term)
        V_tilde_terminal = self.f_theta(t_term_norm, tf.concat([P_term_norm, Y_term_norm], axis=1))

        # Linear penalty, dtype-safe
        penalty = - tf.maximum(tf.constant(0.0, dtype=self.dtype), Y_term - L) * \
                  (tf.constant(1.0, dtype=self.dtype) / self.V_scale)
...
def create_objective_function(PIATrainer, DGMNet, domain_bounds, tf_hammersley_sampler,
    ...
    def objective(trial):
        ...
        k_start         = trial.suggest_float('k_start', 1.0, 2.0, step=0.1)
...
# CLI
def parse_args():
    parser = argparse.ArgumentParser(description="Run HJB PIA Solver with Optuna tuning")
