...
# -*- coding: utf-8 -*-
"""
Multi-objective Optuna tuning for the 3-D HJB solver (Policy-Iteration-Aux).
...
class PIATrainer:
    def __init__(self, value_model, control_model, optimizer_f, optimizer_g,
        ...
        self.domain_bounds = domain_bounds  # [(t_min, t_max), (P_min, P_max), (Y_min, Y_max)]
        ...
        self.terminal_bounds = domain_bounds[1:]
    ...
    def _build_models(self):
        ...
        xy = tf.zeros((bs, 2), dtype=self.dtype)
        _ = self.f_theta(t, xy)
        _ = self.g_phi(t, xy)
    ...
    def _setup_normalization(self):
        t_bounds, P_bounds, Y_bounds = self.domain_bounds
    ...
    def normalize_inputs(self, t, P, Y):
        ...
        return t_norm, P_norm, Y_norm

    def get_V_and_alpha(self, t, P, Y):
        t_norm, P_norm, Y_norm = self.normalize_inputs(t, P, Y)
        V = self.f_theta(t_norm, tf.concat([P_norm, Y_norm], axis=1))
        alpha = self.g_phi(t_norm, tf.concat([P_norm, Y_norm], axis=1))
        return V, alpha
    ...
    @tf.function
    def compute_value_loss(self, t, P, Y, t_term, P_term, Y_term):
        ...
        drift = mu * alpha * P * V_P
        diffusion = (sigma**2 * alpha**2 * P**2 * V_PP_safe) / tf.constant(2.0, dtype=self.dtype)
        advec_Y = C * (a + b * P) * V_Y
        source_term = pi(P)

        V = tf.cast(V, self.dtype)
        residual = V_t + drift + diffusion + advec_Y + source_term - r * V
        pde_loss = tf.reduce_mean(tf.square(residual))

        t_term_norm, P_term_norm, Y_term_norm = self.normalize_inputs(t_term, P_term, Y_term)
        V_terminal = self.f_theta(t_term_norm, tf.concat([P_term_norm, Y_term_norm], axis=1))

        # Terminal penalty (no scaling needed)
        penalty = - tf.maximum(tf.constant(0.0, dtype=self.dtype), Y_term - L)
    ...
    @tf.function
    def compute_control_loss(self, t, P, Y):
        # Compute V, V_P, V_PP
        with tf.GradientTape(persistent=True) as tape2:
            tape2.watch([t, P, Y])
            with tf.GradientTape(persistent=True) as tape1:
        ...
        # No scaling needed for derivatives or control
        V_P_orig  = V_P
        V_PP_orig = V_PP_safe
        alpha_orig = alpha

        # Optimal control (with P in denominator)
        alpha_optimal_orig  = -mu * V_P_orig / (sigma**2 * P * V_PP_orig)
        ...
        H = (mu * alpha_orig * P * V_P_orig) + \
            (tf.constant(0.5, dtype=self.dtype) * sigma**2 * alpha_orig**2 * P**2 * V_PP_orig)
    ...
    def resample_from_residual(self, step, batch_size=4096):
        t_cand, P_cand, Y_cand = tf.split(self.candidates, 3, axis=1)
        self.k = tf.constant(self.get_scheduled_k(step), dtype=self.dtype)

        t_term_cand, P_term_cand, Y_term_cand = self.sample_terminal(self.candidate_size)
        ...
        for i in range(num_batches):
            ...
            t_b = t_cand[start:end]; P_b = P_cand[start:end]; Y_b = Y_cand[start:end]
            t_term_b = t_term_cand[start:end]; P_term_b = P_term_cand[start:end]; Y_term_b = Y_term_cand[start:end]

            _, _, _, _, residual_b, terminal_error_b, VPP_penalty_vec_b = self.compute_value_loss(
                t_b, P_b, Y_b, t_term_b, P_term_b, Y_term_b
            )
        ...
        return (tf.split(selected, 3, axis=1),
    ...
    def sample_terminal(self, n):
        points = tf_hammersley_sampler(n, self.terminal_bounds, dtype=self.dtype)
        P, Y = tf.split(points, 2, axis=1)
    ...
    def train(self, steps=5000, terminal_batch_size=512, log_every=10):
        for step in trange(1, steps + 1, desc="Training"):
            if step % self.resample_every == 1:
                try:
                    (t_int, P_int, Y_int), max_pde_residual, max_terminal_residual, max_VPP_penalty, max_total_loss = \
                ...
                tqdm.write(
                    ...
                    f"Max neg V_PP penalty: {max_VPP_penalty:.2e}, "
            ...
            with tf.GradientTape() as tape_g:
                control_loss, mean_integrand = self.compute_control_loss(t_int, P_int, Y_int)
...
def create_objective_function(PIATrainer, DGMNet, domain_bounds, tf_hammersley_sampler,
                              mu, sigma, C, a, b, pi, r, L,
...
def optimize_hyperparameters(PIATrainer, DGMNet, domain_bounds, tf_hammersley_sampler,
                             mu, sigma, C, a, b, pi, r, L,
                             n_trials=100, seed=3, training_steps=1000,
                             candidate_size=100_000, dtype=tf.float32):

    objective = create_objective_function(
        PIATrainer, DGMNet, domain_bounds, tf_hammersley_sampler,
        mu, sigma, C, a, b, pi, r, L,
...
def train_with_best_params(study, PIATrainer, DGMNet, domain_bounds, tf_hammersley_sampler,
                           mu, sigma, C, a, b, pi, r, L,
...
# === Pipeline ===
def run_optimization_pipeline(PIATrainer, DGMNet, domain_bounds, tf_hammersley_sampler,
                              mu, sigma, C, a, b, pi, r, L,
    ...
    trainer = train_with_best_params(
        study, PIATrainer, DGMNet, domain_bounds, tf_hammersley_sampler,
        mu, sigma, C, a, b, pi, r, L,
    ...
    study_path = f"optuna_study_3d_{timestamp}.pkl"
...
# CLI
def parse_args():
    parser = argparse.ArgumentParser(description="Run 3D HJB PIA Solver with Optuna tuning - Simplified")
    ...
    parser.add_argument("--experiment_name", type=str, default="3d_experiment_simplified")
...
def main():
    ...
    global mu, sigma, C, a, b, r, T, L, domain_bounds, seed
    mu = tf.constant(args.mu, dtype=dtype)
    sigma = tf.constant(args.sigma, dtype=dtype)
    C = tf.constant(args.C, dtype=dtype)
    ...
    # Correct Y bound for constant-C dynamics: dY = C*(a + bP) dt
    P_max = tf.constant(10.0, dtype=dtype)
    Y_max = C * (a + b * P_max) * T
    domain_bounds = [(0.0, T.numpy()), (0.1, 10.0), (0.0, Y_max.numpy())]

    seed = args.seed
    os.environ['PYTHONHASHSEED'] = str(seed)
    tf.random.set_seed(seed); np.random.seed(seed); random.seed(seed)

    study, trainer = run_optimization_pipeline(
        PIATrainer, DGMNet, domain_bounds, tf_hammersley_sampler,
        mu, sigma, C, a, b, pi, r, L,
    ...
    metadata = {
        ...
        "scaling": "No V_scale or alpha_scale - all equations use natural units"
    }
